 ...
 *   adapts to the session's own history (predicted midpoint vs actual) and
 *   persists across sessions via localStorage (cross-session calibration).
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ------------------------------------------------------------------
    // Cross-session calibration: persist actual-vs-predicted samples in
    // localStorage so later sessions start from an informed prior. All reads
    // are guarded (private browsing / storage disabled) and capped.
    // ------------------------------------------------------------------
    const STORAGE_KEY = "dsh-cost-estimate:calibration:v1";
    const MAX_PERSISTED_SAMPLES = 100;
    ...
    function loadCalibrationSamples() {
      try {
        ...
        return parsed
          .filter((s) => typeof s?.mid === "number" && typeof s?.actual === "number")
          .slice(-MAX_PERSISTED_SAMPLES);
      } catch {
        return [];
      }
    }

    let persistedSamples = loadCalibrationSamples();

    function saveCalibrationSamples(samples) {
      try {
        if (typeof window === "undefined" || window.localStorage === undefined) return;
        const next = [...persistedSamples, ...samples].slice(-MAX_PERSISTED_SAMPLES);
        persistedSamples = next;
        window.localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
      } catch {
        /* storage unavailable — calibration stays in-session only */
      }
    }
    ...
    const estimateDefinition = {
      ...
      start(context, match, reader) {
        ...
        // Cross-session prior (persisted) blended with this session's history.
        const calFactor = calFactorOf([...persistedSamples, ...(prevState?.history ?? [])].slice(-16));
    ...
    function CostEstimateNodeView({ node, t }) {
      const d = node.data;
      // Persist calibration samples once the actual arrives (debounced).
      // Side effects are allowed in the renderer; the definition fold stays
      // replay-deterministic.
      const lastSavedHistory = react.useRef(undefined);
      react.useEffect(() => {
        if (d.phase !== "actual" || !Array.isArray(d.history) || d.history.length === 0) return;
        if (d.history === lastSavedHistory.current) return;
        lastSavedHistory.current = d.history;
        const timer = setTimeout(() => saveCalibrationSamples(d.history), 800);
